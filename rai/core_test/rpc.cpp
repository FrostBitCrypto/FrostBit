...
TEST (rpc, history)
{
	...
	rai::genesis genesis;
	rai::state_block usend (rai::genesis_account, node0->latest (rai::genesis_account), rai::genesis_account, rai::genesis_amount - rai::Gfrost_ratio, rai::genesis_account, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
	...
	ASSERT_EQ (rai::Gfrost_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (rai::Gfrost_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, payment_end_nonempty)
{
	...
	auto node1 (system.nodes[0]);
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	system.wallet (0)->init_free_accounts (rai::transaction (node1->store.environment, false));
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", rai::amount (rai::Mfrost_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Mfrost_ratio);
	...
	request1.put ("amount", rai::amount (rai::Mfrost_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
TEST (rpc, search_pending)
{
	...
	ASSERT_EQ (rai::process_result::progress, system.nodes[0]->ledger.process (rai::transaction (system.nodes[0]->store.environment, true), block).code);
	rai::rpc rpc (system.service, *system.nodes[0], rai::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, mrai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::Mfrost_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", rai::Mfrost_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, krai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::kfrost_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", rai::kfrost_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, rai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::frost_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, rai_from_raw)
{
	...
	request1.put ("amount", rai::frost_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, bootstrap)
{
	...
	{
		rai::transaction transaction (system1.nodes[0]->store.environment, true);
...
TEST (rpc, wallet_change_seed)
{
	...
	rai::keypair seed;
	{
		rai::transaction transaction (system0.nodes[0]->store.environment, false);
		rai::raw_key seed0;
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (rai::genesis_amount - rai::Gfrost_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	rai::genesis genesis;
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gfrost_ratio));
	...
	request.put ("representative", rai::test_genesis_key.pub.to_account ());
	request.put ("balance", rai::Gfrost_ratio.convert_to<std::string> ());
...
TEST (rpc, confirmation_history)
{
	rai::system system (24000, 1);
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (rai::test_genesis_key.pub, rai::test_genesis_key.pub, rai::Gfrost_ratio));
	...
	assert (tally_num == rai::genesis_amount || tally_num == (rai::genesis_amount - rai::Gfrost_ratio));
	system.stop ();
}
