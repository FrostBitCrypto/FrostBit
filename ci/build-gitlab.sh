...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DFROSTBIT_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DFROSTBIT_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DFROSTBIT_SIMD_OPTIMIZATIONS=ON"
    CRYPTOPP_CFG=""
...
if [[ "${BETA}" -eq 1 ]]; then
    NETWORK_CFG="-DACTIVE_NETWORK=fstbt_beta_network"
else
    NETWORK_CFG="-DACTIVE_NETWORK=fstbt_live_network"
fi
