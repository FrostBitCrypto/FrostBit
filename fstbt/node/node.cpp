...
namespace rai
{
extern unsigned char fstbt_bootstrap_weights[];
extern size_t fstbt_bootstrap_weights_size;
}
...
node (node_a),
generator (node_a, rai::fstbt_network == rai::fstbt_networks::fstbt_test_network ? std::chrono::milliseconds (10) : std::chrono::milliseconds (500))
{
}
...
vote_uniquer (block_uniquer)
{
	...
	BOOST_LOG (log) << "Node starting, version: " << FROSTBIT_VERSION_MAJOR << "." << FROSTBIT_VERSION_MINOR;
	...
	if (rai::fstbt_network == rai::fstbt_networks::fstbt_live_network || rai::fstbt_network == rai::fstbt_networks::fstbt_beta_network)
	{
		...
		if (!rai::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mice_ratio, 0, true) << " ice";
...
{
	assert (balance_a >= amount_a * rai::Gice_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= rai::Gice_ratio;
		auto balance_scaled ((balance_l / rai::Mice_ratio).convert_to<double> ());
