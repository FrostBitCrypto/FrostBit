...
receive_minimum (rai::ice_ratio),
online_weight_minimum (60000 * rai::Gice_ratio),
...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::ice_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
			tree_a.put ("version", "4");
			result = true;
		case 4:
...
stats (config.stat_config)
{
	...
	observers.vote.add ([this](MDB_txn * transaction, std::shared_ptr<rai::vote> vote_a, rai::endpoint const & endpoint_a) {
		assert (endpoint_a.address ().is_v6 ());
