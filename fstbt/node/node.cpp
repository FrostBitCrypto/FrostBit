...
receive_minimum (rai::frost_ratio),
online_weight_minimum (60000 * rai::Gfrost_ratio),
...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::frost_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
			tree_a.put ("version", "4");
			result = true;
		case 4:
		...
		if (!rai::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mfrost_ratio, 0, true) << " FROST";
...
{
	assert (balance_a >= amount_a * rai::Gfrost_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= rai::Gfrost_ratio;
		auto balance_scaled ((balance_l / rai::Mfrost_ratio).convert_to<double> ());
