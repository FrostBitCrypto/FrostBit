...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for fstbt_test_network)")
	...
	if (ec == rai::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be fstbt_test_network" << std::endl;
			}
		}
