...
extern "C" {
void frost_uint128_to_dec (frost_uint128 source, char * destination)
{
	auto const & number (*reinterpret_cast<rai::uint128_union *> (source));
	strncpy (destination, number.to_string_dec ().c_str (), 32);
}

void frost_uint256_to_string (frost_uint256 source, char * destination)
{
...
void frost_uint512_to_string (frost_uint512 source, char * destination)
{
...
int frost_valid_address (const char * account_a)
{
...
void frost_generate_random (frost_uint256 seed)
{
	auto & number (*reinterpret_cast<rai::uint256_union *> (seed));
	rai::random_pool.GenerateBlock (number.bytes.data (), number.bytes.size ());
}

void frost_seed_key (frost_uint256 seed, int index, frost_uint256 destination)
{
...
void frost_key_account (const frost_uint256 key, frost_uint256 pub)
{
