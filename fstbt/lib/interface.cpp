...
extern "C" {
void ice_uint128_to_dec (ice_uint128 source, char * destination)
{
	auto const & number (*reinterpret_cast<rai::uint128_union *> (source));
	strncpy (destination, number.to_string_dec ().c_str (), 32);
}

void ice_uint256_to_string (ice_uint256 source, char * destination)
{
...
void ice_uint512_to_string (ice_uint512 source, char * destination)
{
...
int ice_valid_address (const char * account_a)
{
...
void ice_key_account (const ice_uint256 key, ice_uint256 pub)
{
