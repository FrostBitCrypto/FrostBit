...
TEST (timer, states)
{
	rai::timer<std::chrono::milliseconds> t1;
	ASSERT_EQ (t1.current_state (), rai::timer_state::stopped);
	t1.start ();
	ASSERT_EQ (t1.current_state (), rai::timer_state::started);
	t1.restart ();
	ASSERT_EQ (t1.current_state (), rai::timer_state::started);
	t1.pause ();
	ASSERT_EQ (t1.current_state (), rai::timer_state::stopped);
	...
	rai::timer<std::chrono::milliseconds> t2 (rai::timer_state::started);
	ASSERT_EQ (t2.current_state (), rai::timer_state::started);
	t2.stop ();
	ASSERT_EQ (t2.current_state (), rai::timer_state::stopped);
}

TEST (timer, measure_and_compare)
{
	using namespace std::chrono_literals;
	rai::timer<std::chrono::milliseconds> t1 (rai::timer_state::started);
	ASSERT_EQ (t1.current_state (), rai::timer_state::started);
