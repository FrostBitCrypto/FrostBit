...
{
	rai::signature_checker checker (0);
	rai::signature_check_set check = { 0, nullptr, nullptr, nullptr, nullptr, nullptr };
...
TEST (signature_checker, bulk_single_thread)
{
	...
	rai::signature_check_set check = { size, messages.data (), lengths.data (), pub_keys.data (), signatures.data (), verifications.data () };
	checker.verify (check);
...
{
	rai::signature_checker checker (4);
	...
	auto signature_checker_work_func = [&checker]() {
		...
		rai::state_block invalid_block (key.pub, 0, key.pub, 0, 0, key.prv, key.pub, 0);
		...
		std::vector<rai::signature_check_set> signature_checker_sets;
