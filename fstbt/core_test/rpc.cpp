...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		if (!system.nodes[0]->store.account_get (transaction, rai::test_genesis_key.pub, info))
		{
...
TEST (rpc, history)
{
	...
	rai::genesis genesis;
	rai::state_block usend (rai::genesis_account, node0->latest (rai::genesis_account), rai::genesis_account, rai::genesis_amount - rai::Gice_ratio, rai::genesis_account, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
	...
	ASSERT_EQ (rai::Gice_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (rai::Gice_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, payment_wait)
{
	...
	system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Mice_ratio);
...
{
	rai::system system (24000, 2);
	rai::endpoint endpoint (boost::asio::ip::address_v6::from_string ("fc00::1"), 4000);
	system.nodes[0]->peers.insert (endpoint, rai::protocol_version);
	rai::rpc rpc (system.io_ctx, *system.nodes[0], rai::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (std::to_string (rai::protocol_version), peers_node.get<std::string> ("[::1]:24001"));
	...
	ASSERT_EQ (std::to_string (rai::protocol_version), peers_node.get<std::string> (endpoint_text.str ()));
}
...
TEST (rpc, peers_node_id)
{
	...
	ASSERT_EQ (std::to_string (rai::protocol_version), tree1.get<std::string> ("protocol_version"));
	...
	ASSERT_EQ (std::to_string (rai::protocol_version), tree2.get<std::string> ("protocol_version"));
...
TEST (rpc, kice_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::Mice_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, kice_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
...
TEST (rpc, kice_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::kice_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, ice_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::ice_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, ice_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (rai::genesis_amount - rai::Gice_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	rai::genesis genesis;
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gice_ratio));
	...
	request.put ("representative", rai::test_genesis_key.pub.to_account ());
	request.put ("balance", rai::Gice_ratio.convert_to<std::string> ());
...
TEST (rpc, online_reps)
{
	...
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, rai::test_genesis_key.pub, rai::Gice_ratio));
	ASSERT_NE (nullptr, send_block);
	system.deadline_set (10s);
...
TEST (rpc, confirmation_history)
{
	...
	assert (tally_num == rai::genesis_amount || tally_num == (rai::genesis_amount - rai::Gice_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	ASSERT_TRUE (system.nodes[0]->active.confirmed.empty ());
	auto send1 (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gice_ratio));
	auto send2 (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gice_ratio));
	auto send3 (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gice_ratio));
	system.deadline_set (10s);
	...
	rai::amount tally_num;
	tally_num.decode_dec (tally);
	assert (tally_num == rai::genesis_amount || tally_num == (rai::genesis_amount - rai::Gice_ratio) || tally_num == (rai::genesis_amount - 2 * rai::Gice_ratio) || tally_num == (rai::genesis_amount - 3 * rai::Gice_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm)
{
	...
	rai::genesis genesis;
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto send1 (std::make_shared<rai::state_block> (rai::test_genesis_key.pub, genesis.hash (), rai::test_genesis_key.pub, rai::genesis_amount - rai::Gice_ratio, rai::test_genesis_key.pub, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
