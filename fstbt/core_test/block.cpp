...
TEST (block, confirm_req_hash_serialization)
{
	rai::keypair key1;
	rai::keypair key2;
	rai::send_block block (1, key2.pub, 200, rai::keypair ().prv, 2, 3);
...
{
	rai::keypair key;
	rai::keypair representative;
	std::vector<std::pair<rai::block_hash, rai::block_hash>> roots_hashes;
	rai::state_block open (key.pub, 0, representative.pub, 2, 4, key.prv, key.pub, 5);
	roots_hashes.push_back (std::make_pair (open.hash (), open.root ()));
	for (auto i (roots_hashes.size ()); i < 7; i++)
	{
		rai::keypair key1;
		rai::keypair previous;
		rai::state_block block (key1.pub, previous.pub, representative.pub, 2, 4, key1.prv, key1.pub, 5);
		roots_hashes.push_back (std::make_pair (block.hash (), block.root ()));
	}
	roots_hashes.push_back (std::make_pair (open.hash (), open.root ()));
	rai::confirm_req req (roots_hashes);
	std::vector<uint8_t> bytes;
	{
		rai::vectorstream stream (bytes);
		req.serialize (stream);
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
	             ...
	             .work (0)
	             .build (ec);
	ASSERT_EQ (ec, rai::error_common::missing_representative);
}
...
TEST (block_builder, state_errors)
{
	...
	ASSERT_EQ (ec, rai::error_common::bad_account_number);
