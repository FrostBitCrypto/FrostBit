...
TEST (ipc, asynchronous)
{
	...
	rai::ipc::ipc_server ipc (*system.nodes[0], rpc);
	rai::ipc::ipc_client client (system.nodes[0]->io_ctx);

	auto req (client.prepare_request (rai::ipc::payload_encoding::json_legacy, std::string (R"({"action": "block_count"})")));
	auto res (std::make_shared<std::vector<uint8_t>> ());
	std::atomic<bool> call_completed{ false };
	client.async_connect ("::1", 24077, [&client, &req, &res, &call_completed](rai::error err) {
		client.async_write (req, [&client, &req, &res, &call_completed](rai::error err_a, size_t size_a) {
			ASSERT_NO_ERROR (static_cast<std::error_code> (err_a));
			ASSERT_EQ (size_a, req->size ());
			// Read length
			client.async_read (res, sizeof (uint32_t), [&client, &res, &call_completed](rai::error err_read_a, size_t size_read_a) {
				...
				// Read json payload
				client.async_read (res, payload_size_l, [&res, &call_completed](rai::error err_read_a, size_t size_read_a) {
...
TEST (ipc, synchronous)
{
	...
	system.nodes[0]->config.ipc_config.transport_tcp.port = 24077;
	rai::ipc::ipc_server ipc (*system.nodes[0], rpc);
	rai::ipc::rpc_ipc_client client (system.nodes[0]->io_ctx);
